...
def PrintInfos(ProjectID, ConfigDict):
  ...
  Format = "{:13s}: {}"
  print Format.format("ProjectID",     ProjectID)
  print Format.format("ProjectName",   ConfigDict["ProjectName"])
  print Format.format("FlickrProject", ConfigDict["FlickrProject"])
  print Format.format("Year",          ConfigDict["Year"])
  print Format.format("DIR_HOME",      ConfigDict["DIR_HOME"])
  print Format.format("DIR_OUT",       ConfigDict["DIR_OUT"])
  print Format.format("DIR_DATA",      ConfigDict["DIR_DATA"])
  print Format.format("DIR_STORE",     ConfigDict["DIR_STORE"])
  print Format.format("LastUpload",    ConfigDict["LastUpload"])
...
def Jpg2Raw(JpgFile, ProjectName):
  ...
  return RawFile1, RawFile2, RawFile3


def Jpg2Wm(JpgFile):

  Base = os.path.basename(JpgFile).split(".")[0]

  # WmFile = Base + "_wm.jpg"
  WmFile = "{}_wm.jpg".format(Base)
